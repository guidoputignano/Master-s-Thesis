...
A phenomenological model written for this project. Every rate law below is our choice, built from
standard pieces (compartment transfers, tanh and Hill switches, first-order lags, spontaneous plus
recruited conversion, threshold-linear terms) and fitted to published observations of the group's
parallel-plate chamber (observations.py). Where a form matches a classic one (structural kinetics
for J, Finke-Watzky/Bass for the spreading of disorder, excess-shear erosion for the limit), the
match was noticed afterwards (paper, Table S3).

State (fractions of the monolayer, then four scalars)
------------------------------------------------------
...
pi priming: a first-order lag of the across-flow drive
D  junction damage, 0-1 (the published connectivity index is read as 1 - D)
R  cells retained, relative to the start

Mechanisms and the observations behind them
-------------------------------------------
- Shear sets a preferred orientation: along the flow below a switch shear tau_x, across above it
  (Stefopoulos 2022 Fig. 1). Topography adds a bias along its grooves, and gratings start with a
  fraction a_grat of the monolayer ordered along them (Robotti 2014).
- Native domains order toward the preferred orientation; ordered neighbours recruit them, and
  destabilised junctions speed ordering up (beta_j, not identified). Across-flow ordering starts
  after a lag (t_p), fitted mainly to the unchanged orientation 2 h into 8 Pa (Fig. 1g); no
  mechanism is assigned to it.
- An ordered domain facing the opposite preference collapses into frustrated disorder. Collapse is
  fast only while junctions are destabilised (abrupt switches, Fig. 4); with PP1 or after a
  gradual change the order persists and then decays by spreading disorder (Fig. 4l PP1; Fig. S5).
- Frustrated domains re-order only slowly (random 4 and 6 h after the abrupt rise, Fig. 4h); the
  data bound this rate from above only.
- Without shear, ordered domains relax to native at 0.02/h (fixed; no data).
- Collapse through destabilised junctions damages them; its size rests on Wu 2021 Fig. 5. The slow
  spontaneous route does not damage. Following Robotti et al. (2014), who proposed that monolayers
  forced to re-orient at high shear lose integrity while monolayers pre-polarised across the flow
  hold, shear above a limit (per material, higher on gratings) breaks the junctions of domains not
  ordered as it demands (native, frustrated or opposed), in proportion to the excess. An earlier
  version, whose limit acted on every domain, failed the test on gratings across the flow (kept in
  results/). Gratings anchor cells: they reduce the damage and the cell loss that a collapse causes.
  Damage heals where domains are not frustrated.
- Cells are lost while ordered domains collapse (Stefopoulos 2022 Video S5; Wu 2021 Fig. 5), where
  ...
  connected monolayers (Robotti 2014, gratings across the flow at 10 Pa).
- An assumption against, not from, the literature: where junctions are broken, cells revert to
  aligning along the flow (the damage**4 terms). Isolated cells show neither ordered state
  (Stefopoulos 2022); the term helps reproduce gratings along the flow at 10 Pa.
...
PARAMS = {
    ...
    "t_p":        (1.0, 0.05, 4.0, "lag of across-flow ordering (h)"),
...
class Surface:
    ...
    order: float = 1.0         # order factor (1 flat; 0.58 breath figures, from the orientations
                               # after flow reversal, Wu 2021 Fig. 4e)
...
def preference(tau, p, bias=0.0, damage=0.0):
    ...
    Assumption: where junctions are broken, cells revert to aligning along the flow (damage**4).
    Isolated cells show neither ordered state (Stefopoulos 2022), so this is not taken from the
    data; it helps reproduce gratings along the flow at 10 Pa (Robotti 2014)."""
...
KERNEL_ORDER = ("tau_x", "w_x", "k_ord", "rho_c", "kappa", "beta_j", "k_x", "k_col", "j50", "q_hill",
                "t_p", "k_sp", "lg_seed", "tau_j", "t_j", "theta_a", "theta_c", "delta_x",
...
def simulate(shear, theta, surfaces, pp1_mask=None, record_every=1, extra_loss=None):
    ...
    returns    dict of arrays (n_records, n_protocols): N, A, C, X, J, D, R, angle, ar, ci, retention
    """
    ...
    if USE_KERNEL and _kernel.kernel is not None and extra_loss is None:
        ...
        rec = _kernel.kernel(np.ascontiguousarray(shear), Pm, idx, bias.astype(float), f.astype(float),
                             ...
                             pp1, DT, TAU_ACT, N_SINGLE, TAU_REF, K_RELAX_STATIC)
        if record_every != 1:
    ...
    for k in range(n_steps):
        ...
        # order lost per hour through destabilised junctions (the damaging route); on breath figures
        # the domains are less ordered (f), so less order is lost: f enters here once, via s_a, s_c
        ...
        src = prot * p["delta_x"] * collapse + p["k_lim"] * over * mismatched
