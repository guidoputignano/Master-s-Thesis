...
class ModelParameters:
    def __init__(self):
        ...
        # h — aspect-ratio adaptation constant, unified to 7.4 h (= orientation
        # constant; one morphological constant). Governs ASPECT RATIO only — cell
        # AREA is fixed by the tessellation, not relaxed. NOTE: this ModelParameters
        # class is an unused parallel copy; the reported config is SimulationConfig
        # (config.py). Kept in sync to avoid a contradictory stale value.
        self.tau_adapt_hours = 7.4
