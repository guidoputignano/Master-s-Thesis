...
class ModelParameters:
    def __init__(self):
        ...
        self.theta_star = 20.0        # deg — healthy-cell orientation at the 1.4 Pa plateau (Chala et al. 2021)
        # h — aspect-ratio adaptation constant, 3 h (= orientation constant; one
        # morphological constant; plateau at 6-8 h, Stefopoulos et al. 2022).
        # Governs ASPECT RATIO only — cell AREA is fixed by the tessellation, not
        # relaxed. NOTE: this ModelParameters class is an unused parallel copy; the
        # reported config is SimulationConfig (config.py). Kept in sync to avoid a
        # contradictory stale value.
        self.tau_adapt_hours = 3.0
