...
def build(args):
    ...
    for tag, cells in (('v1', cells1), ('v2.1', cells2)):
        ...
        for cond, d in inner.groupby(inner.key.map(folder_of)):
            for stratum, s in d.groupby('stratum'):
                for _, r in bv.pick(s, N_CELL, rng).iterrows():
                    ...
                    items.append(dict(block='cell', method=tag, folder=cond, key=r.key, label=int(r.label),
                                      ...
                                      area_um2=r.area_um2, question=bv.Q_CELL, view0='junctions'))
            ...
            for _, r in bv.pick(multi, N_MULTI, rng).iterrows():
                shown.add((tag, r.key, int(r.label)))
                items.append(dict(block='multinucleated', method=tag, folder=cond, key=r.key, label=int(r.label),
                                  stratum=f'{int(r.n_nuclei)} nuclei', stratum_n=len(multi), area_um2=r.area_um2,
                                  question=Q_MULTI, view0='junctions'))
    ...
    for cond, g in big.groupby(big.key.map(folder_of)):
        for _, r in bv.pick(g, N_ENLARGED, rng).iterrows():
            ...
            items.append(dict(block='enlarged', method='v2.1', folder=cond, key=r.key, label=int(r.label),
                              stratum='posterior>0.5', area_um2=r.area_um2, question=bv.Q_CELL, view0='junctions'))
    for (tag, cond), g in gaps.groupby(['method', 'folder']):
        folder_area = g.area_um2.sum()             # before exclusion: the estimand is this folder's gap area
